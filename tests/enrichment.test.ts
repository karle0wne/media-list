...
test("manual selection is durable before provider enrichment and later becomes canonical", async () => {
  ...
  try {
    const userId = await createTestDataOwner(db);
    const added = await addSelectedMediaToUser(db, userId, provisional);
    assert.equal(added.inserted, true); assert.equal(added.needsEnrichment, true);
    const before = (await db.select().from(media).where(eq(media.id, added.item.id)).limit(1))[0];
    assert.equal(before.title, "Search Result"); assert.equal(before.metadataStatus, "PENDING");
...
test("provider failure keeps the added item and exposes a durable retry state", async () => {
  ...
  try {
    ...
    assert.equal(item.metadataStatus, "ERROR"); assert.equal(item.metadataError, "provider unavailable"); assert.equal((await db.select().from(userMedia).where(eq(userMedia.mediaId, added.item.id))).length, 1);
    ...
    assert.equal(item.metadataStatus, "PENDING"); assert.equal(item.metadataError, null);
  } finally { sqlite.close(); }
});
