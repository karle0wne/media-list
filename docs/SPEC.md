...
- Manual search is category-first. Anime waits only for AniList (plus localized fallback when necessary), movies only for TMDB movie search, books only for Open Library, and TV search first finds shows then loads seasons only for the selected show. Unrelated providers must not add latency to manual search.
...
3. Quick import: paste titles or supported provider URLs (including Cyrillic titles) -> resolve candidates across applicable providers -> review -> save.
