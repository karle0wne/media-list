...
- The application must remain viable on a single small VPS; no Redis, PostgreSQL, queues or microservices are required for the MVP.
...
1. Manual canonical add: provider search -> explicit choice -> provider revalidation -> save.
2. Canonical CSV: strict source/id based round-trip import/export.
