...
`;
type AniMedia = { id: number; idMal?: number | null; title: { romaji?: string | null; english?: string | null; native?: string | null }; countryOfOrigin?: string | null; episodes?: number | null; startDate?: { year?: number | null } | null; coverImage?: { medium?: string | null } | null; description?: string | null };
...
function toCandidate(item: AniMedia): MediaCandidate { const title = item.title.english || item.title.romaji || item.title.native || `AniList #${item.id}`; return { key: `ANILIST:${item.id}:`, type: "ANIME", source: "ANILIST", externalId: String(item.id), externalSubId: "", title, originalTitle: item.title.native ?? item.title.romaji ?? null, countryCode: item.countryOfOrigin ?? null, year: item.startDate?.year ?? null, episodeCount: item.episodes ?? null, coverUrl: item.coverImage?.medium ?? null, description: item.description?.replace(/<[^>]+>/g, "") ?? null }; }
